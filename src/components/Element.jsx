...
export default function Element({ elementData, series, from }) {
  ...
  if (elementData && elementData.category === "alkaline earth metal")
    color = "#f97927";
  if (elementData && elementData.category === "alkali metal") color = "#fdc235";
  if (elementData && elementData.category === "transition metal")
    color = "#2685c1";
  if (elementData && elementData.category === "post-transition metal")
    color = "#263d51";
  if (elementData && elementData.category === "polyatomic nonmetal")
    color = "#ea2d59";
  if (elementData && elementData.category === "noble gas") color = "#8b58ad";
  if (elementData && elementData.category === "diatomic nonmetal")
    color = "#4caed4";
  if (elementData && elementData.category === "metalloid") color = "#769191";
  if (
    elementData &&
    elementData.category === "unknown, probably post-transition metal"
  )
    color = "#f542da";
  if (
    elementData &&
    elementData.category === "unknown, probably transition metal"
  )
    color = "#c5f542";
  if (elementData && elementData.category === "unknown, probably metalloid")
    color = "#ffd4f7";
  if (
    elementData &&
    elementData.category === "unknown, predicted to be noble gas"
  )
    color = "#d5a6bd";
  ...
  return (
    <div
      ...
      className={
        elementData
          ? `bg-${color}-900 cursor-pointer hover:bg-gray-100 rounded`
          : ""
      }
